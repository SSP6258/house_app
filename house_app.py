...
def fn_gen_plotly_hist(fig, data, title, row=1, col=1, margin=None, bins=100, color_by=None, line_color='white', showlegend=False,
    ...
    fig.add_trace(
        go.Histogram(x=data, name=title, showlegend=showlegend, nbinsx=bins, hovertext=hovertext, color=color_by,
                     marker=dict(
                         opacity=opacity,
                         # color=color_by,
                         line=dict(
...
def fn_gen_analysis_statistic(df):
    ...
    fig_bar = fn_gen_plotly_hist(fig_bar, df['交易月'], '交易月', row=1, col=2, bins=50, margin=margin, color_by='交易年')
